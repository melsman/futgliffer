...
structure Json :> JSON = struct
  ...
  withtype obj = (string,t) BM.dict

  (* Operations on object maps *)
  fun objFromList l = List.foldl (fn ((k,v),obj) => BM.insert(obj,k,v)) (BM.mkDict String.compare) l
  fun objFromKeyValues l = List.foldl (fn ((k,v),obj) => BM.insert(obj,k,STRING v)) (BM.mkDict String.compare) l
  fun objLook obj k = BM.peek(obj,k)
  fun objFold (f : (string * t) * 'a -> 'a) (acc : 'a) (obj:obj) : 'a =
      BM.foldl (fn (k,v,a) => f((k,v),a)) acc obj
  fun objList obj = BM.listItems obj
  fun objAdd obj k v = BM.insert(obj,k,v)
  val objEmp : (string,t)BM.dict = BM.mkDict String.compare
