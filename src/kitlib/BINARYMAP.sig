...
signature BINARYMAP =
  sig
    ...
    val peekGreatestLesseq : ('key, 'a) dict * 'key -> ('key*'a) option
  end
...
(* 
   ...
   [peekGreatestLesseq (m,k)] returns SOME v if there exists a key k' in
   the map such that k' is associated with v and k'<=k. Returns NONE
   otherwise.
*)
