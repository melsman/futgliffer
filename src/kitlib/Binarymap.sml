...
structure Binarymap : BINARYMAP =
  struct
	...
	fun peekGreatestLesseq (DICT(cmpKey, t), x) =
	    let fun merge NONE p = SOME p
                  | merge (c as SOME(k1,v1)) (k2,v2) =
                    case cmpKey (k2,k1) of
                        GREATER => (case cmpKey(x,k2) of
                                        LESS => c           
                                      | _ => SOME(k2,v2))   (* k1 < k2 <= x *)
                      | _ => c
                fun findCandidate E candidate = candidate
		  | findCandidate (T(n as {key,left,right,...})) candidate =
		    case cmpKey (x,key) of
			GREATER => findCandidate right (merge candidate (key,#value n))   (* key is a candidate *)
		      | LESS    => findCandidate left candidate
		      | EQUAL   => SOME (key,#value n)
	    in findCandidate t NONE end
